...
historic_rank.dt[, .(min_date = min(date), max_date = max(date),
...
historic_rank.dt <- historic_rank.dt[historic_rank_week_plus.dt, ] 
historic_rank.dt <- historic_rank.dt[historic_rank_month_plus.dt, ] 
...
historic_rank.dt[, c("date_week_plus", "date_month_plus"):= NULL]

setnames(historic_rank.dt, c("ID", "Rank", "Average", "Bayes average", "Users rated", "date",
                             "i.Rank", "i.Average", "i.Bayes average", "i.Users rated", 
                             "i.Rank.1", "i.Average.1", "i.Bayes average.1", "i.Users rated.1"),
                           c("ID", "Rank", "Rating", "Geek_Score", "Users_Rated", "Date",
...
historic_rank.dt[!is.na(prior_week_Rank), `:=`(WoW_Rank = (prior_week_Rank - Rank), WoW_Rating = (Rating - prior_week_Rating),
                                               WoW_Geek_Score = (`Geek_Score` - `prior_week_Geek_Score`), WoW_Users_Rated = (`Users_Rated` - `prior_week_Users_Rated`))]
historic_rank.dt[!is.na(prior_month_Rank), `:=`(MoM_Rank = (prior_month_Rank - Rank), MoM_Rating = (Rating - prior_month_Rating),
                                                 MoM_Geek_Score = (`Geek_Score` - `prior_month_Geek_Score`), MoM_Users_Rated = (`Users_Rated` - `prior_month_Users_Rated`))]

historic_rank.dt[, `:=`(WoW_Users_Rated_Percent = WoW_Users_Rated / prior_week_Users_Rated,
                        MoM_Users_Rated_Percent = MoM_Users_Rated / prior_month_Users_Rated)]



monthly_data.dt <- historic_rank.dt[day(Date) == 1, c(1:6,19:22,24)]
monthly_data.dt <- na.omit(monthly_data.dt)
weekly_data.dt <- historic_rank.dt[wday(Date) == 1, c(1:6,15:24)]
...
for(set_type in type) {
  ...
  top_n_type_increase_wow <- setorderv(weekly_data.dt, c(primary, secondary), c(-1, -1))[, head(.SD, n), by = Date, .SDcols = cols]
  top_n_type_increase_wow[, Type := paste0("Increase in ", set_type)]
  top_n_type_decrease_wow <- setorderv(weekly_data.dt, c(primary, secondary), c(1, 1))[, head(.SD, n), by = Date, .SDcols = cols]
  top_n_type_decrease_wow[, Type := paste0("Decrease in ", set_type)]
  WoW_type <- rbind(top_n_type_increase_wow, top_n_type_decrease_wow)
  rm(top_n_type_increase_wow, top_n_type_decrease_wow)
...
for(set_type in type) {
  ...
  top_n_type_increase_mom <- setorderv(weekly_data.dt, c(primary, secondary), c(-1, -1))[, head(.SD, n), by = Date, .SDcols = cols]
  top_n_type_increase_mom[, Type := paste0("Increase in ", set_type)]
  top_n_type_decrease_mom <- setorderv(weekly_data.dt, c(primary, secondary), c(1, 1))[, head(.SD, n), by = Date, .SDcols = cols]
  top_n_type_decrease_mom[, Type := paste0("Decrease in ", set_type)]
  MoM_type <- rbind(top_n_type_increase_mom, top_n_type_decrease_mom)
  rm(top_n_type_increase_mom, top_n_type_decrease_mom)
...
for(category in major_categories) {
  ...
  for(set_type in type) {
    primary <- paste0("WoW ", set_type)
    secondary <- if(set_type == "Geek Score") {"WoW Rating"} else {"WoW Geek Score"}
    top_n_type_increase_wow <- setorderv(historic_rank_category, c(primary, secondary), c(-1, -1))[, head(.SD, n), by = Date, .SDcols = cols]
    top_n_type_increase_wow[, Type := paste0("Increase in ", set_type)]
    top_n_type_decrease_wow <- setorderv(historic_rank_category, c(primary, secondary), c(1, 1))[, head(.SD, n), by = Date, .SDcols = cols]
    top_n_type_decrease_wow[, Type := paste0("Decrease in ", set_type)]
...
for(category in major_categories) {
  ...
  for(set_type in type) {
    primary <- paste0("MoM ", set_type)
    secondary <- if(set_type == "Geek Score") {"MoM Rating"} else {"MoM Geek Score"}
    top_n_type_increase_MoM <- setorderv(historic_rank_category, c(primary, secondary), c(-1, -1))[, head(.SD, n), by = Date, .SDcols = cols]
    top_n_type_increase_MoM[, Type := paste0("Increase in ", set_type)]
    top_n_type_decrease_MoM <- setorderv(historic_rank_category, c(primary, secondary), c(1, 1))[, head(.SD, n), by = Date, .SDcols = cols]
    top_n_type_decrease_MoM[, Type := paste0("Decrease in ", set_type)]
